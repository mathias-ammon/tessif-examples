...
import tessif_examples.data.frused.namedtuples as nts
from tessif_examples.data.frused.configurations import mimos
from tessif_examples.data.frused.paths import doc_dir
from tessif_examples.data.write.log import logging_levels
...
def to_csv(iterable, name, columns=4, path=None, **kwargs):
    ...
    if path is None:
        path = os.path.join(
            doc_dir, "source", "api", "frused", "spellings", f"{name}.csv"
        )
...
def get_from(dct, smth_like, dflt=None):
    ...
    logger.debug(50 * "-")
    logger.debug(f"Try getting a key similar to {smth_like}...")
    ...
    if smth_like in globals():
        ...
        for variation in globals()[smth_like]:
            if variation in dct.keys():
                ...
                logger.debug(f"... which matches to {dct[variation]}")
                logger.debug(50 * "-")
